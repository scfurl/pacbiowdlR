...
pub fn read_bed_with_group(path: &std::path::Path) -> Result<Vec<Region>> {
    ...
    while reader.read_record(&mut record)? != 0 {
        ...
        if end > start {
            // filter out invalid contigs
            let valid_chroms = vec![
            ...
            if valid_chroms.contains(&chrom.as_str()) {
                out.push(Region { chrom, start, end, name, group: group.clone() , coverage: None } );
            }
        }
    }
...
pub fn read_bigwig(path: &str, mut regions: Vec<Region>)    -> Result<Vec<Region>> {
    let mut reader = BigWigRead::open_file(path).unwrap();
    // eprintln!("{:?}", reader.chroms());
    for region in &mut regions {
        let mut values = Vec::new();
        for value in reader.get_interval(&("chr".to_string() + &region.chrom), region.start as u32, region.end as u32)? {
            values.push(value.unwrap());
        }
        region.coverage = Some(values.iter().map(|v| v.value as f32).collect());
    }
        return Ok(regions);
}
