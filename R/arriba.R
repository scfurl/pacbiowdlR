  #' Draw fusion transcripts
  #'
  #' This function reproduces the logic of the original script that draws fusion transcripts,
  #' including validation of inputs, optional coverage, protein domains, circos plots, etc.
  #'
  #' @param fusionsFile A path to the fusion TSV (Arriba or STAR-Fusion output).
  #'   Default: \code{"fusions.tsv"}.
  #' @param exonsFile A path to the GTF annotation file.
  #'   Default: \code{"annotation.gtf"}.
  #' @param outputFile The name of the PDF file to produce.
  #'   Default: \code{"output.pdf"}.
  #' @param alignmentsFile A path to the sorted BAM file. If empty, coverage is not plotted.
  #'   Default: \code{"Aligned.sortedByCoord.out.bam"}.
  #' @param cytobandsFile A path to a \code{cytobands.tsv} file for drawing ideograms and circos plots.
  #'   Default: \code{"cytobands.tsv"}.
  #' @param minConfidenceForCircosPlot Minimum confidence level for fusions to be shown in circos plot. Must be one of \code{"none"}, \code{"low"}, \code{"medium"}, or \code{"high"}.
  #'   Default: \code{"medium"}.
  #' @param proteinDomainsFile A path to a GFF3 of protein domain annotations.
  #'   Default: \code{"protein_domains.gff3"}.
  #' @param sampleName A sample name to display as the page title.
  #'   Default: \code{""} (no title).
  #' @param squishIntrons Whether introns should be visually "squished" to a constant length. Logical.
  #'   Default: \code{TRUE}.
  #' @param printExonLabels Whether to parse and display the \code{exon_number} field in GTF. Logical.
  #'   Default: \code{TRUE}.
  #' @param render3dEffect Whether to draw the 3D shading effect on exons/ideograms. Logical.
  #'   Default: \code{TRUE}.
  #' @param pdfWidth Numeric width of the produced PDF.
  #'   Default: \code{11.692}.
  #' @param pdfHeight Numeric height of the produced PDF.
  #'   Default: \code{8.267}.
  #' @param color1 HEX or R color string used for gene1.
  #'   Default: \code{"#e5a5a5"}.
  #' @param color2 HEX or R color string used for gene2.
  #'   Default: \code{"#a7c4e5"}.
  #' @param mergeDomainsOverlappingBy Numeric fraction in (0,1) for how much overlap is required to merge two protein domains.
  #'   Default: \code{0.9}.
  #' @param optimizeDomainColors Whether to re-assign domain colors to maximize visual contrast. Logical.
  #'   Default: \code{FALSE}.
  #' @param fontSize Overall font size in the plots.
  #'   Default: \code{1}.
  #' @param fontFamily Which font family to use in the PDF.
  #'   Must be available in \code{pdfFonts()}. Default: \code{"Helvetica"}.
  #' @param showIntergenicVicinity Either numeric (how many bases around an intergenic breakpoint) or special keywords (\code{"closestGene"}, \code{"closestProteinCodingGene"}) that cause dynamic determination of range.
  #'   Default: \code{"0"} (no extra region).
  #' @param transcriptSelection One of \code{"coverage"}, \code{"provided"}, or \code{"canonical"} controlling how to choose transcripts for the breakpoints.
  #'   Default: \code{"provided"}.
  #' @param fixedScale If positive, forcibly scale transcripts to fit that many bases in the horizontal space. If 0, scales automatically.
  #'   Default: \code{0}.
  #' @param coverageRange A character string with up to two numeric values (e.g., \code{"0"} or \code{"10,20"}) controlling coverage normalization.
  #'   "0" means automatic. If two values are given, each is used for gene1 and gene2 separately.
  #'   Default: \code{"0"}.
  #'
  #' @return Produces a PDF with the plots. Invisibly returns \code{NULL}.
  #' @examples
  #' \dontrun{
  #'   draw_fusions(
  #'     fusionsFile = "fusions.tsv",
  #'     exonsFile = "annotation.gtf",
  ...
  #' @importFrom graphics polygon text rect lines legend plot dev.off par pdf
  #' @importFrom grDevices pdfFonts pdf dev.off col2rgb rgb
  #' @import GenomicRanges
  #' @import circlize
  #'
  #' @export
  draw_fusions <- function(
      fusionsFile = "fusions.tsv",
      exonsFile = "annotation.gtf",
      outputFile = "output.pdf",
      alignmentsFile = "Aligned.sortedByCoord.out.bam",
      cytobandsFile = "cytobands.tsv",
      minConfidenceForCircosPlot = "medium",
      proteinDomainsFile = "protein_domains.gff3",
      sampleName = "",
      squishIntrons = TRUE,
      printExonLabels = TRUE,
      render3dEffect = TRUE,
      pdfWidth = 11.692,
      pdfHeight = 8.267,
      color1 = "#e5a5a5",
      color2 = "#a7c4e5",
      mergeDomainsOverlappingBy = 0.9,
      optimizeDomainColors = FALSE,
      fontSize = 1,
      fontFamily = "Helvetica",
      showIntergenicVicinity = "0",
    ...
    # -------------------------------
    # All code from the original script below,
    # except for argument parsing from the command line
    # and quit() calls
    # -------------------------------

    options(warn = 1)  # print warnings as they happen

    # Validate user inputs (partial checks that used to be in the script):
    if (!file.exists(fusionsFile)) {
      stop(paste("Cannot read file:", fusionsFile))
    }
    if (!file.exists(exonsFile)) {
      stop(paste("Cannot read file:", exonsFile))
    }
    if (!(minConfidenceForCircosPlot %in% c("none", "low", "medium", "high"))) {
      stop("Invalid argument to minConfidenceForCircosPlot")
    }

    # handle the coverageRange input
    coverageRangeVals <- suppressWarnings(as.numeric(unlist(strsplit(coverageRange, ",", fixed=TRUE))))
    if (!(length(coverageRangeVals) %in% 1:2) || any(is.na(coverageRangeVals)) || any(coverageRangeVals < 0)) {
      stop("Invalid argument to coverageRange")
    }

    # check that the chosen font is available
    if (!(fontFamily %in% names(pdfFonts()))) {
      stop(paste0("Unknown font: ", fontFamily, ". Available fonts: ", paste(names(pdfFonts()), collapse=", ")))
    }

    # old script code: color helper functions
    changeColorBrightness <- function(color, delta) {
      cvals <- col2rgb(color, alpha = TRUE)
      rgb(
        min(255, max(0, cvals["red", ]   + delta)),
        min(255, max(0, cvals["green", ] + delta)),
        min(255, max(0, cvals["blue", ]  + delta)),
        cvals["alpha", ],
        maxColorValue = 255
      )
    }
    ...
    # read fusions
    fusions <- read.table(fusionsFile, stringsAsFactors = FALSE, sep = "\t", header = TRUE,
                          comment.char = "", quote = "")
    if (nrow(fusions) == 0) {
      pdf(outputFile, onefile = TRUE, width = pdfWidth, height = pdfHeight, title = ifelse(sampleName != "", sampleName, fusionsFile), family = fontFamily)
    ...
    # Different format checks (Arriba, STAR-Fusion, etc.)
    if (colnames(fusions)[1] == "X.gene1") {
      ...
      fusions$transcript_id1 <- ifelse(!("CDS_LEFT_ID" %in% colnames(fusions)), ".", fusions$CDS_LEFT_ID)
      fusions$transcript_id2 <- ifelse(!("CDS_RIGHT_ID" %in% colnames(fusions)), ".", fusions$CDS_RIGHT_ID)
      fusions$fusion_transcript <- ifelse(!("FUSION_CDS" %in% colnames(fusions)), ".",
                                          toupper(sub("([a-z]*)", "\\1|", fusions$FUSION_CDS, perl=TRUE)))
      fusions$reading_frame <- ifelse(!("PROT_FUSION_TYPE" %in% colnames(fusions)), ".",
      ...
      # determine structural rearrangement type
      fusions$type <- ifelse(fusions$contig1 != fusions$contig2, "translocation",
                             ifelse(fusions$direction1 == fusions$direction2, "inversion",
                                    ifelse((fusions$direction1 == "downstream") == (fusions$breakpoint1 < fusions$breakpoint2),
    ...
    # check cytobands
    cytobands <- NULL
    if (nzchar(cytobandsFile)) {
      if (!file.exists(cytobandsFile)) {
        warning(paste("Cannot read cytobands file:", cytobandsFile, "\nNo ideograms/circos plots will be drawn."))
      } else {
    ...
    exons$exonNumber <- if (printExonLabels) parseGtfAttribute("exon_number", exons) else ""

    message("Loading protein domains")
    proteinDomains <- NULL
    if (nzchar(proteinDomainsFile)) {
      if (!file.exists(proteinDomainsFile)) {
        warning(paste("Cannot read protein domains file:", proteinDomainsFile))
        proteinDomains <- NULL
      } else {
        proteinDomains <- scan(proteinDomainsFile,
                               what=list(contig="",src="",type="",start=0,end=0,score="",
    ...
    for (fusion in 1:nrow(fusions)) {
      ...
      if (is.null(cytobands) || !("circlize" %in% names(sessionInfo()$otherPkgs)) || !("GenomicRanges" %in% names(sessionInfo()$otherPkgs))) {
        ...
        drawCircos(fusion, fusions, cytobands, minConfidenceForCircosPlot, circosColors, fontSize, getBrightColor)
  ...
  drawCircos <- function(fusion, fusions, cytobands, minConfidenceForCircosPlot, circosColors, fontSize, getBrightColor) {
    # check if Giemsa staining information is available
    cytobands <- cytobands[cytobands$contig %in% c(1:22, "X", "Y"),]
