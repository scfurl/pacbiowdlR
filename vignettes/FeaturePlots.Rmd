...
ComplexHeatmap::Heatmap(
    ...
    use_raster = T,
    raster_quality = 1,
    top_annotation = columnAnnotation(signal=anno_points(colMeans(amf, na.rm = T)))
  )
...
ggplot(data.frame(signal=colMeans(amf, na.rm = T), position=1:ncol(amf)), aes(x=position, y=signal))+geom_line()+theme_void()
