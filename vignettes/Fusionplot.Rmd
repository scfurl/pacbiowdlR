...
Okay, that is a seemingly overwhelming number of gene-gene fusions...  We can filter the fusion calls to known fusions

```{r, eval=TRUE}
db_fusions <- get_neosplice_db()

results <- find_related_fusions(fusions, db_fusions)
hits <- which(lapply(results, function(f) length(f)>0) %>% unlist()) %>% as.numeric()
fusions[hits,]
print_fusion_matches(results[hits])

fusions <- get_fusions_from_vcf(sv_file, gtf_file = gtf, plot = T, filter = "", highlight = hits)
```
